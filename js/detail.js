...
app.controller('detail', function($scope) {
  ...
  var max = source[car.index].dishes[car.index].max == 0 ?
      source[car.index].dishes[car.index].list.length :
      source[car.index].dishes[car.index].max;

  $scope.max = max;
  $scope.count = source[car.index].dishes[car.index].list.length;
  ...
  $scope.selected = function(index, obj) {
    ...
    if ($(cur).find('.checked').length > 0) {
      ...
      if ($scope.select > 0) {
        $scope.select--;
        //delAlbum(source[car.index].dishes[car.index].list[index].title);
      }
    } else {
      //添加残品至购物车
